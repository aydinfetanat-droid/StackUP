    ...
    <div className="min-h-screen bg-ink-50">
      <header className="bg-ink-950 px-6 pb-6 pt-8 text-white">
        ...
        <div className="mt-6 flex items-center gap-3">
          <div className="flex flex-1 items-center justify-between rounded-md border border-white/15 px-4 py-3">
            <span className="text-sm text-white/70">{currentRank.title}</span>
            <span className="text-sm tabular-nums text-white/50">{rankProgress.percent}%</span>
          </div>
          <div className="flex items-center gap-1.5 rounded-md border border-white/15 px-3 py-3 tabular-nums">
      ...
      <main className="px-5 pt-6">
          ...
          <section className="mb-8">
            ...
            <button onClick={() => navigate(`/lesson/${nextLesson.id}`)} className="card tap w-full p-4 text-left elevate-sm">
              <div className="flex items-center gap-4">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-md bg-ink-900 text-base font-semibold tabular-nums text-white">
                  {nextLesson.order}
                </div>
        ...
        {!loading && !nextLesson && (
          <div className="mb-8 rounded-lg border border-dashed border-ink-300 bg-white p-4 text-center">
        ...
        <section className="mb-8">
          <div className="flex items-center justify-between">
            <p className="label-caps">Today's signals</p>
            <button onClick={() => navigate("/signals")} className="text-xs font-semibold text-ink-500 hover:text-ink-900">
              See all
            </button>
          </div>
          <div className="mt-3 flex flex-col gap-2">
            {news.map((item) => (
              <button key={item.id} onClick={() => navigate("/signals")} className="card tap flex w-full items-center gap-3 p-3.5 text-left">
